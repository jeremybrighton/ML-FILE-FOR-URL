...
# Enable CORS globally with preflight support for dashboard
CORS(
    app,
    origins=["https://fraud-detector-topaz.vercel.app"],  # your frontend domain
...
def process_dataset():
    ...
    try:
        ...
        df["fraud_score"] = results_df["fraud_score"]
        df["risk_level"] = results_df["risk_level"]
        # Save each row to MongoDB
        for i, row in df.iterrows():
            record = row.to_dict()
            transactions_col.insert_one(record)
        return jsonify({"message": f"{len(df)} transactions processed successfully", "predictions": df.to_dict(orient="records")}), 200
    except Exception as e:
...
def explain_transaction(transaction_id):
    ...
    try:
        ...
        # Prepare DataFrame for SHAP (example, actual SHAP logic can be more complex)
        df = pd.DataFrame([txn])
        # Placeholder: Return feature importance (replace with actual SHAP computation)
        feature_importance = {col: random.random() for col in df.columns if col not in ["_id"]}
        return jsonify({"transaction_id": transaction_id, "feature_importance": feature_importance}), 200
    except Exception as e:
