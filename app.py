...
def predict():
    ...
    for tx in transactions:
        ...
        fraud_score = 0.65  # placeholder for ML model
...
# -----------------------------
# MongoDB
MONGO_URI = os.environ.get("MONGO_URI")  # Set in Render Environment Variables
if not MONGO_URI:
    raise ValueError("MONGO_URI environment variable not set!")

# Email (OTP)
SENDER_EMAIL = os.environ.get("SENDER_EMAIL")      # Example: your_email@gmail.com
SENDER_PASSWORD = os.environ.get("SENDER_PASSWORD")  # App password for Gmail or SMTP
...
try:
    ...
    logging.error(f"❌ Failed to load ML model or features: {e}")
    exit(1)
...
def predict_internal(new_data: pd.DataFrame):
    ...
    risk_level = pd.Series(np.where(prob < 0.2, 'LOW', np.where(prob < 0.8, 'MEDIUM', 'HIGH')),
...
def send_email_otp(to_email, otp_code):
    ...
    body = f"Your one-time login code is: {otp_code}. It expires in {OTP_EXPIRY_MINUTES} minutes."
    msg = MIMEText(body)
    msg['Subject'] = subject
...
def login_user():
    ...
    if check_password_hash(user["password"], password):
        users_col.update_one({"email": email}, {"$push": {"login_attempts": {"status":"success", "timestamp": datetime.utcnow()}}})
        return jsonify({"message": "Login successful"}), 200
    else:
        users_col.update_one({"email": email}, {"$push": {"login_attempts": {"status":"failed", "timestamp": datetime.utcnow()}}})
